...
func (m *RawIngestRequest) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *RawIngestRequest) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *RawIngestRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.Batch) > 0 {
		i -= len(m.Batch)
		copy(dAtA[i:], m.Batch)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Batch)))
		i--
		dAtA[i] = 0x12
	}
	if m.PdpId != nil {
...
func (m *RawIngestRequest) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *RawIngestRequest) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: RawIngestRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: RawIngestRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
